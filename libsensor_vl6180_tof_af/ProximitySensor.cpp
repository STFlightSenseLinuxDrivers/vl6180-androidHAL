...
int ProximitySensor::enable(int32_t, int en) {
    ...
    //Added for debug
    ALOGE("ProximitySensor::enable function called for stmvl5180!");
    ...
    if (flags != mEnabled) {
        ...
        if (fd >= 0) {
            ...
            mEnabled = flags;
            setInitialState();
       LOGE("ProximitySensor::enable enabled VL6180! Error code(2 should indicate success): %d flag %d", err, flags);
            return 0;
        }
        LOGE("ProximitySensor::enable: Failed to open file descriptor for enabling VL6180");
        LOGE("ProximitySensor::enable: Attempted to open file %s. Error: %d", PROXIMITY_ENABLE_FILENAME, fd);
        return -1;
    }
    return 0;
}
